/***************************************************
* Laboratory Arduino-tank. Version 1.2  (17.03.18) *
...
const unsigned long ShortTime = 200; // Время реакции на пульт мсек
...
int speedRightIndex = 0; // Индекс скорости
int speedRightIndOld = 0; // Старый индекс скорости
int speedLeftIndex = 0; // Индекс скорости
int speedLeftIndOld = 0; // Старый индекс скорости
...
void moveback() {
  ...
  delay(Rotate45 * 4 - (StartTime + StopTime));
...
void ReadIR() {
  if (irrecv.decode(&results)){
    speedRightIndOld = speedRightIndex;
    speedLeftIndOld = speedLeftIndex;
#ifdef TEST_MODE
      Serial.println(results.value);
#endif    
    switch (results.value) {
        case 0x41BEA05F: // Поднимаем скорость
          if (speedRightIndOld < 5) speedRightIndex++;
          if (speedLeftIndOld < 5) speedLeftIndex++;
          break;
        case 0x41BEE01F: // Гасим скорость
          if (speedRightIndOld > -5) speedRightIndex--;
          if (speedLeftIndOld > -5) speedLeftIndex--;
          break;
        case 0x41BE10EF: // Вправо
          if (speedRightIndOld < 1) speedRightIndex++;
          if (speedLeftIndOld > -5) speedLeftIndex--;
          break;
        case 0x41BEC837: // Влево
          if (speedRightIndOld > -5) speedRightIndex--;
          if (speedLeftIndOld < 1) speedLeftIndex++;
          break;
        case 0x41BE906F: // Стоп
          speedRightIndex = 0;
          speedLeftIndex = 0;
          break;
        default:
    ...
    irrecv.resume();
    if (speedRightIndex == speedLeftIndex) myservo.write(ServoCenter);
    else myservo.write(ServoCenter + 80 * (speedRightIndex - speedLeftIndex) / max(abs(speedRightIndex), abs(speedLeftIndex)));
  }
}

void MoveIR() {
  uint8_t speedRightFwd = 0;
  uint8_t speedLeftFwd = 0;
  uint8_t speedRightBwd = 0;
  uint8_t speedLeftBwd = 0;
  if (speedRightIndex > 0) {
    speedRightFwd = map(speedRightIndex, 0, 5, MinSpd, MaxSpd);
  } else if (speedRightIndex < 0) {
    speedRightBwd = map(speedRightIndex, 0, -5, MinSpd, MaxSpd);
  }
  if (speedLeftIndex > 0) {
    speedLeftFwd = map(speedLeftIndex, 0, 5, MinSpd, MaxSpd);
  } else if (speedRightIndex < 0) {
    speedLeftBwd = map(speedLeftIndex, 0, -5, MinSpd, MaxSpd);
  }
  analogWrite(motorRightForward, speedRightFwd);
  analogWrite(motorRightBackward, speedRightBwd);
  analogWrite(motorLeftForward, speedLeftFwd);
  analogWrite(motorLeftBackward, speedLeftBwd);
}
